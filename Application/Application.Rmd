...
```{r,echo=T}
pred.dist.age.lopm<-CalibInc(mod.calib,pred=lopm.alldist,aggregate=~dist+age)
pred.dist.age.lopm
...
```{r,echo=T}
cr.dist.age.lopm<-CalibInc(mod.calib,pred=lopm.alldist,aggregate=~dist+age,
...
### 3. Calculate the SIR and add prediction intervals thanks to
`LogNormPI` function ###
```{r,echo=T}
